...
### Step 2.5. ラッコの構成データ（任意）

ラッコキーワードで「見出し抽出・共起語・Q&A/PAA」を取得済みなら、CSV/JSON を `tmp/rakko/<slug>/` に置いてもらう。**あれば競合カバーと網羅性が上がる。無くても従来どおり動く。**

置き場所・エクスポート項目・解釈ルールは `.claude/config/rakko-data-format.md` を参照。**スクレイピングはしない**（公式エクスポートのみ）。

### Step 3. 構成案（人間の承認ポイント・省略禁止）

`writer` エージェントを呼び、構成案を作らせる。**Step 2.5 のラッコデータがあればフォルダのパスを渡す。**
...
`editor` エージェントを呼ぶ。**Step 4.5 の実測値を必ず渡す。** Step 2.5 のラッコデータ（共起語・狙いKW）があれば、それも渡してカバレッジを検査させる。

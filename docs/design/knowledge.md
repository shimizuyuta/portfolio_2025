...
章の切れ目をスクロール中に一目で分かるようにする。`ArticleBody.tsx` の `prose-h2:` / `prose-h3:` ユーティリティで定義（公開ページと `/admin` プレビューの双方に自動適用）。
※`.prose h2` の生CSSは Tailwind v4 では出力されないため、ユーティリティ方式を使う。

- **h2**: 下罫線（gray-200）＋ 短い sky アクセント下線（`after:` で `w-12`・`h-[3px]`・`rounded-full`・`bg-sky-500`）。
- **h3**: 左 sky バー（`border-l-4 border-sky-500`）＋ 左パディング（`pl-3`）。

色はリンク（`sky-600`）と同系の sky でまとめ、既存のクリーン／モダンな雰囲気を保つ。ネイビー帯（案A）や太い左バー（案C）は不採用。
